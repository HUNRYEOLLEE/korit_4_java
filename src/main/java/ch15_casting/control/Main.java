...
public class Main {
    public static void main(String[] args) {
        ...
        // 배열 생성 방법을 떠올려서 5개짜리 사이즈의 배열을 CentralControl() 생성자의 argument로 집어넣어보세요.


        Power[] powers = {computer1, led1, mouse1};
        CentralControl control2 = new CentralControl(powers);
        // 빈 배열을 집어넣는 것만 가능한게 아니라, 미리 배열을 생성해놓고 (근데 14번 라인을 확인했을 때 이미 암시적 업캐스팅이 이루어짐), 그 배열을
        // control2 생성시에 집어넣는 방법이 가능하다.
        // 다만 위의 경우 만약 구글홈미니를 만들어놨는데 거기에 default로
        // 컴퓨터, Led, 마우스가 한대씩 등록되어있다고 봐야하기 때문에
        // 9번 라인에 작성한 control1을 기준으로 한다.

        control1.addDevice(computer1);
        //System.out.println(control1.checkEmpty());      // 결과값 : 1 -> 확인 후 checkEmpty() 메서드를 private로 설정하였음
        ...
        // tv1의 경우 control의 필드에 들어가있지 않기 때문에
        tv1.channelUp();
