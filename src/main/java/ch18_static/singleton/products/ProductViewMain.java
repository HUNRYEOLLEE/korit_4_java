...
//        ProductView productView2 = new ProductView();     // 기본 생성자 private 선언 후에는 오류 발생

        // 생성자를 private 처리를 했고, getInstance() 메서드 내에서 instance 정적 변수가 null 이라면
        // instance = new ProductVIew(); 하기로 정의했었기 때문에
        // main 단계에서 ProductView.getInstance(); 를 통해 객체를 생성한다.
        ...
        /*
            위의 코드에서 복수의 객체를 생성한 것처럼 보이지만 실제로 ProductView.getInstance();의 결과값은
            동일한 instance 이다.
            그렇다면 실제 생성된 객체의 수는 '1개'이다.
            그리고 그 한 개를 다른 객체 이름에 대입을 했을 뿐이다.
