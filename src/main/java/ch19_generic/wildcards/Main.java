...
public class Main {
    // AnimalData의 필드가 Animal의 서브 클래스에 해당하는 지 확인하는 메서드를 정의
    // wildcard가 적용되기 때문에 주의깊게 봐야함
...
//        animalData3.printData();
        ...
        /*
            25번 라인에서 오류가 생긴다는 점을 확인할 수 있다.
            AnimalData 클래스의 .printData() 메서드를 호출하는 과정에서
            Car 객체(Animal의 필드인)를 강제로 Animal 클래스로 형변환 하려고 했기 때문에 발생하는 예외인데
            결국 필드에 Car 객체가 대입은 되지만 메서드는 호출되지 않는다는 점을 확인할 수 있다.

            그렇다면 애초에 Car 객체가 필드에 대입되지 못하도록 설정할 필요성이 대두되는데,
            이 것이 와일드 카드 개념의 의의이다.
        ...
        // 위의 코드를 작성했을 때 생성자를 통해서 객체 생성을 하지 않았다는 점,
        // 직접 getAnimal() 메서드를 정의하지 않았다면 어떤 클래스의 객체가 argument로 들어갔는지
        // 모른다는 점 등의 잠재적인 문제가 있음

        // 그래서 오류를 발생시키지않기 위해 조건문을 작성
        if(animalData4 != null) {
